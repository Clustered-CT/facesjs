...
import featureDescriptions from "../svgs/faces_descriptions.json" with { type: "json" };
...
const drawFeature = (svg, face, info) => {
  ...
  if (["hat", "hat2", "hat3"].includes(face.accessories.id) && info.name == "hair") {
    if (["afro", "afro2", "curly", "curly2", "curly3", "faux-hawk", "hair", "high", "juice", "messy-short", "messy", "middle-part", "parted", "shaggy1", "shaggy2", "short3", "spike", "spike2", "spike3", "spike4"].includes(face.hair.id)) {
      face.hair.id = "short";
    } else if (["blowoutFade", "curlyFade1", "curlyFade2", "dreads", "fauxhawk-fade", "tall-fade"].includes(face.hair.id)) {
  ...
  for (let i = 0; i < info.positions.length; i++) {
    ...
    const element = svg.lastChild;
    lastElement = element;
    element.setAttribute("data-feature", info.name);
    element.setAttribute("data-feature-index", String(i));
    const tag = info.tags?.[i] ?? (info.tags?.length === 1 ? info.tags[0] : undefined) ?? (info.positions.length > 1 ? `${info.name}-${i}` : info.name);
    if (tag) {
...
export const display = (container, face, overrides) => {
